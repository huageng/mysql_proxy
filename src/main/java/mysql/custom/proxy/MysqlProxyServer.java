...
public class MysqlProxyServer {
    ...
    public static void main(String[] args) {
        if (args.length == 0){
            logger.error("invalid param port and host");
        }
        String host = args[0];
        Integer port = Integer.parseInt(args[1]);
        Vertx.vertx().deployVerticle(new MysqlProxyServerVerticle(port, host));
    }
    ...
    public static class MysqlProxyServerVerticle extends AbstractVerticle {
        ...
        public MysqlProxyServerVerticle(Integer port, String host){
            this.port = port;
            this.mysqlHost = host;
        }
    ...
    public static class MysqlProxyConnection {
        ...
        private void proxy() {      //当代理与mysql服务器连接关闭时，关闭client与代理的连接
            serverSocket.closeHandler(v -> clientSocket.close());      //反之亦然
            clientSocket.closeHandler(v -> serverSocket.close());      //不管那端的连接出现异常时，关闭两端的连接
